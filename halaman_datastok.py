...
# Function to check if data exists for a specific date in the database
def data_exists(tanggal):
    conn = connect_to_database()
    if conn:
        cursor = conn.cursor()
        query = "SELECT COUNT(*) FROM forecasting WHERE TGL_TRANSAKSI = %s"
        cursor.execute(query, (tanggal,))
        count = cursor.fetchone()[0]  # Get the number of rows
        conn.close()
        return count > 0
    return False
...
def add_data_from_csv(file):
    try:
        ...
        st.error(f"Failed to add data from CSV: {e}")
