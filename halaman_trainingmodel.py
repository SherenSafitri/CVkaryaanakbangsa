...
def train_hw_model(data, conn, nama_barang):
    ...
    # Ramalan dengan Holt-Winters
    hw_forecast = hw_fit.forecast(steps=10)
    
    # Visualisasi
    st.subheader('Ramalan Holt-Winters')
...
def train_arima_model(data, conn, nama_barang):
    ...
    # Ramalan dengan ARIMA
    arima_forecast = arima_fit.forecast(steps=10)
    
    # Visualisasi
    st.subheader('Ramalan ARIMA')
    st.line_chart(pd.DataFrame({
        'Actual Data': data['JUMLAH_STOK'],
...
def show_halaman_trainingmodel():
    ...
    if uploaded_file is not None:
        ...
        if st.button("Latih Model"):
            ...
            st.subheader(f"Perbandingan Hasil Ramalan untuk {selected_product}")
            ...
            # Tambahkan penjelasan grafik di bawah kode perbandingan hasil ramalan
            st.write("""
            Penjelasan Grafik Perbandingan Hasil Ramalan:
            - **Holt-Winters Forecast**: Garis biru menunjukkan ramalan menggunakan model Holt-Winters.
            Holt-Winters adalah metode eksponensial smoothing yang cocok untuk data dengan komponen musiman.
            - **ARIMA Forecast**: Garis oranye menunjukkan ramalan menggunakan model ARIMA.
